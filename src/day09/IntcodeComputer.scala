...
case class IntcodeComputer(data: Vector[Long],
  ...
  @tailrec
  final def runUntilOut(): (IntcodeComputer, Seq[Long]) = {
    if (out.isEmpty) {
      data(ptr) match {
        case 99 => (this, Seq())
  ...
  @tailrec
  final def debug(): Seq[Long] = {
    data(ptr) match {
      case 99 =>
        println(out)
        out
      case _ =>
        println("ptr = " + this.ptr + " relBase = " + this.relBase)
        //        println("current position = ..." + this.data.slice(this.ptr, this.ptr + 5).mkString(", "))
        println("out = " + this.out)
        println("data = " + this.data)
        step().debug()
    }
  }

  def put(in: Int): IntcodeComputer = {
